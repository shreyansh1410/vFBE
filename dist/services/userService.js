"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.getUserByClerkId = exports.ensureUserExists = void 0;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
const ensureUserExists = (clerkId) => __awaiter(void 0, void 0, void 0, function* () {
    let user = yield prisma.user.findUnique({ where: { clerkId } });
    if (!user) {
        const clerkUser = { email: `user_${clerkId}@example.com`, name: "User" };
        user = yield prisma.user.create({
            data: {
                clerkId,
                email: clerkUser.email,
                name: clerkUser.name,
            },
        });
    }
    return user;
});
exports.ensureUserExists = ensureUserExists;
const getUserByClerkId = (clerkId) => __awaiter(void 0, void 0, void 0, function* () {
    yield (0, exports.ensureUserExists)(clerkId);
    return prisma.user.findUnique({
        where: { clerkId },
        include: { appliedJobs: true, savedJobs: true },
    });
});
exports.getUserByClerkId = getUserByClerkId;
